...
class ActivePrivateChatSession:
    ...
    def _update_state_machine_locator_db_field_lazy(self, locator: Locator | None) -> None:
        if locator:
            locator_payload = locator.dict(by_alias=True)
            if self.session_db_object.state_machine_locator != locator_payload:
                self.session_db_object.state_machine_locator = locator_payload
