...
DEFAULT_TG_CHAT_ID, DEFAULT_TG_USER_ID, DEFAULT_TG_USERNAME = 90001, 4114, 'ivan_petrov'
...
def test_save_last_update_info():
    ...
    assert Locator.parse_obj(session_db_dump.state_machine_locator) == Locator('/')
...
def test_save_and_restore_state():
    ...
    assert Locator.parse_obj(session_db_dump.state_machine_locator) == Locator('/another/')
...
def test_switch_state_on_tg_update():
    ...
    session_db_dump = SessionModel.objects.get(tg_chat_id=DEFAULT_TG_CHAT_ID)
    assert Locator.parse_obj(session_db_dump.state_machine_locator) == Locator('/first/', params={'counter': 8})
...
def test_raise_on_process_tg_update_with_wrong_tg_update_object(wrong_update_payload):
    SessionModel.objects.create(
        tg_chat_id=DEFAULT_TG_CHAT_ID,
        tg_user_id=DEFAULT_TG_USER_ID,
        state_machine_locator={'state_class_locator': '/'},
    )
